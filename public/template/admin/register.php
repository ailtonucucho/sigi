...
if(isset($_REQUEST['insert']))
{
	...
	if(!empty($name) && !empty($email) && !empty($pass)  && !empty($dob) && !empty($phone))
	{
		...
		if($result)
			{
				$msg='Admin <?= $langArray["Register"] ?> Successfully';


			}
			else
			{
				$error='* Not <?= $langArray["Register"] ?> Admin Try Again';
			}
	}
	else{
...
<head>
        ...
        <title>Ventura - <?= $langArray["Register"] ?></title>
							...
							<div class="login-right-wrap">
								...
								<form method="post">
									...
									<div class="form-group mb-0">
										<input class="btn btn-primary btn-block" type="submit" name="insert" Value="<?= $langArray["Register"] ?>">
									</div>
								</form>
								<!-- /Form -->
								...
								<!-- Social <?= $langArray["Login"] ?> -->
								<div class="social-login">
									<span><?= $langArray["Register"] ?> with</span>
								...
								<div class="text-center dont-have">Already have an account? <a href="index.php"><?= $langArray["Login"] ?></a></div>
							</div>
                        </div>
                    </div>
                </div>
